...
public interface GameSessionRepository extends JpaRepository<GameSession, Long> {
    ...
    @Query("select coalesce(sum(s.durationMinutes), 0) from GameSession s "
            + "where s.userId = :userId and s.gameId = :gameId and s.status = com.devops.backend.session.SessionStatus.FINISHED")
    ...
    @Query("select s.gameId as gameId, coalesce(sum(s.durationMinutes), 0) as totalMinutes from GameSession s "
            + "where s.userId = :userId and s.status = com.devops.backend.session.SessionStatus.FINISHED "
            + "group by s.gameId")
